...
const Dashboard = () => {
  ...
  const showVideoWidget = settings?.widgets?.videoWidget ?? true;
  const showAccessButton = settings?.widgets?.accessButton ?? true;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={cn("space-y-6", showVideoWidget ? "" : "lg:col-span-3")}>
          <BarrierControls />
          {showAccessButton && <OneTimeAccessDialog />}
          <SystemStatus />
        </div>
      </div>
    </div>
